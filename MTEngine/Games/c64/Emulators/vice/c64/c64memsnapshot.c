...
int c64_snapshot_write_module(snapshot_t *s, int save_roms, int save_reu_data)
{
    snapshot_module_t *m;
    ...
    return cartridge_snapshot_write_modules(s, save_reu_data);
}

int c64_snapshot_read_module(snapshot_t *s, int read_reu_data)
{
    BYTE major_version, minor_version;
    snapshot_module_t *m;
    ...
    if (cartridge_snapshot_read_modules(s, read_reu_data) < 0) {
        return -1;
    }
