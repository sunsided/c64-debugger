...
{
	//LOGD("snapshot_write_byte: %d (at %d)", data, ftell(f));
    if (fputc(data, f) == EOF)
        return -1;
...
{
	//LOGD("snapshot_write_word: %d (at %d)", data, ftell(f));
...
{
	//LOGD("snapshot_write_dword: %d (at %d)", data, ftell(f));
...
//	LOGD("module name: %d", ftell(s->file));
    if (snapshot_write_padded_string(s->file, name, (BYTE)0,
                                     SNAPSHOT_MODULE_NAME_LEN) < 0)
		return NULL;
	
//	LOGD("module maj: %d", ftell(s->file));
    if (snapshot_write_byte(s->file, major_version) < 0)
		return NULL;
	
//	LOGD("module min: %d", ftell(s->file));
	if (snapshot_write_byte(s->file, minor_version) < 0)
		return NULL;
	
//	LOGD("module size: %d", ftell(s->file));
	if (snapshot_write_dword(s->file, 0) < 0)
        return NULL;
	
//	LOGD("module size2: %d sizeof(DWORD)=%d", ftell(s->file), sizeof(DWORD));
...
//	LOGD("snapshot_module_open: first_module_offset=%d", s->first_module_offset);
...
//		LOGD("module maj: %d", ftell(s->file));
		if (snapshot_read_byte(s->file, major_version_return) < 0)
			goto fail;
		
//		LOGD("maj=%d", major_version_return);
		
//		LOGD("module min: %d", ftell(s->file));
		if (snapshot_read_byte(s->file, minor_version_return) < 0)
			goto fail;
		
//		LOGD("min=%d", minor_version_return);
		
//		LOGD("module size: %d", ftell(s->file));
		if (snapshot_read_dword(s->file, &m->size))
            goto fail;
...
int snapshot_module_close(snapshot_module_t *m)
{
    ...
    if (m->write_mode)
	{
//		LOGD(">> size offset=%d", m->size_offset);
        if (fseek(m->file, m->size_offset, SEEK_SET) < 0)
			return -1;
	
//		LOGD(">> module write size at %d (size=%d)", ftell(m->file), m->size);
		if (snapshot_write_dword(m->file, m->size) < 0)
			return -1;
	}
                            ...
                            const char *snapshot_machine_name)
{
	...
	LOGD("snapshot_create: first_module_offset=%d", s->first_module_offset);
                          ...
                          const char *snapshot_machine_name)
{
	...
	LOGD("snapshot_open: first_module_offset=%d", s->first_module_offset);
