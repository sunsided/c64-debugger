...
int crc32_is_initialized = 0;

unsigned long crc32_buf_win32(const char *buffer, unsigned int len)
{
    int i, j;
    unsigned long crc, c;
    const char *p;
...
unsigned long crc32_file_win32(const char *filename)
{
    FILE *fd;
    char *buffer;
    unsigned int len;
    ...
    if (fread(buffer, len, 1, fd) == 1)
        crc = crc32_buf_win32(buffer, len);
                                 ...
                                 const char *filename, unsigned int read_only)
{
    ...
    if (event_image_include) {
        ...
        strcpy(&event_data[2], "");
        *(unsigned long *)(event_data + 3) = crc32_file_win32(filename);
...
static void event_playback_attach_image(void *data, unsigned int size)
{
    ...
    if (*orig_filename == 0) {
        ...
        if (event_image_append(orig_filename, &filename, 0) != 0) {
            ...
            } while (filename != NULL && crc_to_attach != crc32_file_win32(filename));
            if (filename == NULL) {
