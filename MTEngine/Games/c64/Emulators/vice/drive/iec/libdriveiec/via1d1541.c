...
BYTE peek_via1d1541_pra(via_context_t *via_context);
BYTE peek_via1d1541_prb(via_context_t *via_context);
...
BYTE c64d_via1d1541_peek(drive_context_t *ctxptr, WORD addr)
{
	...
	{
		return peek_via1d1541_pra(ctxptr->via1d1541);
	}
	
	if (addr_via == VIA_PRB)
	{
		via_context_t *via_context = ctxptr->via1d1541;
		BYTE byte = peek_via1d1541_prb(via_context);
