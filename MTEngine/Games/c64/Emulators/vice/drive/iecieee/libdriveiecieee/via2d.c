...
BYTE peek_via2d_pra(via_context_t *via_context);
BYTE peek_via2d_prb(via_context_t *via_context);
...
BYTE c64d_via2d_peek(drive_context_t *ctxptr, WORD addr)
{
	...
	{
		return peek_via2d_pra(ctxptr->via2);
	}
	
	if (addr_via == VIA_PRB)
	{
		via_context_t *via_context = ctxptr->via2;
		BYTE byte = peek_via2d_prb(via_context
								   );
...
BYTE drive_writeprotect_sense_peek(drive_t *dptr);
...
BYTE peek_via2d_prb(via_context_t *via_context)
{
	...
	byte = ((rotation_sync_found(via2p->drive)
			 | drive_writeprotect_sense_peek(via2p->drive))
			& ~(via_context->via[VIA_DDRB]))
	| (via_context->via[VIA_PRB] & via_context->via[VIA_DDRB]);
