...
//		                      (palette[i] & 0x000000ff));
		...
		Uint32 c = ((palette[i] & 0x00ff0000) >> 16)
					| ((palette[i] & 0x0000ff00) >> 8)
					| ((palette[i] & 0x000000ff));
...
{
	LOGD("a-video_sw UpdatePaletteLookup");
...
//	else
	...
	SDL_VIDEO_UpdatePaletteLookup(mode, TRUE); //SDL_VIDEO_screen->format->BitsPerPixel == 32);
}

void SDL_VIDEO_SW_PaletteUpdate(void)
{
	LOGD("SDL_VIDEO_SW_PaletteUpdate");
